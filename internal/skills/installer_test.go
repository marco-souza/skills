...
`
	...
	t.Run("handles missing skill", func(t *testing.T) {
		t.Parallel()
		src := t.TempDir()
		if err := os.MkdirAll(filepath.Join(src, DefaultSkillsDir, SkillsSubDir), 0o755); err != nil {
...
`
		...
		if err := os.WriteFile(filepath.Join(mainDir, SkillFileName), []byte(mainContent), 0o644); err != nil {
			t.Fatalf("writing main skill file: %v", err)
		}
...
`
			...
			if err := os.WriteFile(filepath.Join(d, SkillFileName), []byte(content), 0o644); err != nil {
				t.Fatalf("writing skill file for %s: %v", name, err)
			}
		}
...
func TestInstaller_installAllScripts(t *testing.T) {
	...
	t.Run("copies scripts directory", func(t *testing.T) {
		...
		if err := os.MkdirAll(scriptsSrc, 0o755); err != nil {
			t.Fatalf("creating scripts source dir: %v", err)
		}
		if err := os.WriteFile(filepath.Join(scriptsSrc, "common.sh"), []byte("#!/bin/bash\n"), 0o755); err != nil {
			t.Fatalf("writing common script: %v", err)
		}
...
func TestInstaller_installScripts(t *testing.T) {
	...
	t.Run("dedup logic", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(skillDir, "scripts", "shared.sh"), []byte("#!/bin/bash\n"), 0o755); err != nil {
			t.Fatalf("writing shared script: %v", err)
		}
...
func TestCopyDir(t *testing.T) {
	...
	t.Run("nested dirs", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(nestedDir, "deep.txt"), []byte("deep"), 0o644); err != nil {
			t.Fatalf("writing deep file: %v", err)
		}
